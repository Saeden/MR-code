...
If search_k is not provided, it will default to n * n_trees where n is the number of approximate nearest neighbors. Otherwise, search_k and n_trees are roughly independent, 
i.e. the value of n_trees will not affect search time if search_k is held constant and vice versa. Basically it's 
recommended to set n_trees as large as possible given the amount of memory you can afford, and it's recommended to set search_k as large as possible 
...
def ann(query_mesh, feature_list, num_of_trees=1000, top_k=10, search_k=-1, query=False, metric='euclidean'):
    ...
    if not os.path.exists('./query_forest.ann'):
    ...
    # if we already have a forest in our system, load it so that we don't have to build a new one
        ...
        results = a.get_nns_by_vector(mesh_feat, top_k + 1, search_k=search_k, include_distances=True)
...
def test():
    ...
    shapes = ann(mesh_name, feature_list)
